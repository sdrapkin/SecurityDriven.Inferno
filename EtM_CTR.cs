	...
	public static partial class EtM_CTR
	{
		...
		public static void Encrypt(byte[] masterKey, ArraySegment<byte> plaintext, byte[] output, int outputOffset, ArraySegment<byte>? salt = null, uint counter = 1)
		{
			...
			try
			{
				using (var aes = _aesFactory())
				{
					...
					Kdf.SP800_108_Ctr.DeriveKey(hmacFactory: _hmacFactory, key: masterKey, label: salt, context: new ArraySegment<byte>(_contextBuffer.Value, 0, CONTEXT_TWEAK_LENGTH), derivedOutput: new ArraySegment<byte>(_sessionKey.Value), counter: counter);
					...
					using (var ctrTransform = new Cipher.AesCtrCryptoTransform(_encKey.Value, new ArraySegment<byte>(_counterBuffer.Value)))
					{
		...
		public static void Decrypt(byte[] masterKey, ArraySegment<byte> ciphertext, ref ArraySegment<byte>? outputSegment, ArraySegment<byte>? salt = null, uint counter = 1)
		{
			...
			try
			{
				...
				if (outputSegment == null) outputSegment = new ArraySegment<byte>?(new ArraySegment<byte>(new byte[cipherLength]));
